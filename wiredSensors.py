...
def readWiredSensors(bmp280, hdc1080):
    ...
    if (config.BMP280_Present):	
        try:
            ...
            state.currentBarometricPressure = round(old_div(bmp280.read_pressure(),1000), 5)
            ...
            state.currentSeaLevel = round(old_div(bmp280.read_sealevel_pressure(config.BMP280_Altitude_Meters),1000), 5)
        except:
            if (config.SWDEBUG):
